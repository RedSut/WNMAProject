...
class TrekMeshService : Service() {
    ...
    // Tracks incoming FILE payloads: payloadId -> temp File (set on receive, pre-Android 10)
    private val incomingFiles = mutableMapOf<Long, File>()
    // Android 10+: payloadId -> Payload.File (asJavaFile() returned null)
    private val incomingPayloadFiles = mutableMapOf<Long, com.google.android.gms.nearby.connection.Payload.File>()
    ...
    // Maps filePayloadId -> (forwardMsg, sourceEndpointId) for relaying images after transfer completes
    private val pendingRelayForMessage = mutableMapOf<Long, Pair<MeshMessage, String>>()
    ...
    private val payloadCallback = object : PayloadCallback() {
        override fun onPayloadReceived(endpointId: String, payload: Payload) {
            when (payload.type) {
                ...
                Payload.Type.FILE -> {
                    // Always store Payload.File; read via asParcelFileDescriptor() after transfer
                    val payloadFile = payload.asFile() ?: return
                    incomingPayloadFiles[payload.id] = payloadFile
                }
                else -> {}
            }
        }
        ...
        override fun onPayloadTransferUpdate(endpointId: String, update: PayloadTransferUpdate) {
            ...
            val payloadFile = incomingPayloadFiles.remove(update.payloadId)

            val saved = if (payloadFile != null) {
                try {
                    ...
                    Log.w(LOG_TAG, "FILE payload copy failed: ${e.message}")
                    false
                }
            } else false
            ...
            if (saved) {
                ...
                pendingRelayForMessage.remove(update.payloadId)?.let { (forward, sourceEndpointId) ->
                    val withImage = forward.copy(imagePath = dest.absolutePath)
                    forwardToAllExcept(withImage, excludeEndpointId = sourceEndpointId)
                }
            }
        }
    }
    ...
    private fun handleIncomingMessage(endpointId: String, raw: String) {
        ...
        if (ttl > 1) {
            // Forward text immediately; image will be relayed once the file transfer completes
            ...
            if (filePayloadId != 0L) pendingRelayForMessage[filePayloadId] = Pair(forward, endpointId)
        }
    }
